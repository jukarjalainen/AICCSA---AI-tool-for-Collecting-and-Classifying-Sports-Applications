...
- At least one user group must be true when not_relevant=false
...
- Out-of-scope gambling/casino: If the app is primarily casino/card/lottery gameplay (e.g., poker, blackjack, rummy, baccarat, teen patti, slots, roulette, bingo, keno) or generic gambling platforms not tied to sports competitions, set not_relevant=true.
   This includes club/tables, chips/coins, RNG fairness, anti-cheating, multi-table play, casinos, and generic tournaments that are not sports events. Do not attempt to force these into Betting/Predictions/Fantasy_sports; they’re out of scope.
- Out-of-scope wellbeing–social: Relationship/dating/parenting/life-admin apps where the primary goal is social connection or couple communication (daily prompts, games/quizzes, date tracking) without structured health behavior training (e.g., meditation/breathwork/CBT modules) → not_relevant=true.
   Exception: If the app’s core is clinically oriented mental-health training (mindfulness courses, CBT exercises, breathwork programs) or symptom logging for mental health, classify under purpose per rules (often Training/Tracking) and sport_type="mental_wellbeing".
- Out-of-scope motivation/quotes: Apps whose primary function is delivering inspirational quotes/affirmations/reminders (widgets, shareables) without structured health behavior training (e.g., meditation/breathwork/CBT modules) or personal health tracking → not_relevant=true.

# User groups
- Athlete: individuals actively participating (players, exercisers, students/athletes) → Set athlete=true
...
- Betting: real-money wagering on odds/outcomes of sports or esports competitions (bets/odds/parlays/stakes/deposit/withdraw/cash out) or player performance (e.g., more/less on projected stats).
...
- Casino/card/lottery gambling (poker/blackjack/slots/rummy/bingo/roulette/keno) → not_relevant=true (even if they say “tournaments” or “clubs”).
...
- Training: structured instruction (plans/drills/coaching/tutorials; tactics/rotations/playbooks).
  We treat “training” broadly: structured practice to improve a capacity (strength, endurance, or mental regulation). Guided meditation/breathwork is instruction-first, so Training fits.
...
- Meditation/mindfulness/breathwork/stretching apps with guided sessions, courses, or daily programs → Training (even if some streaks/badges exist).
- Mood/habit/journal-first apps (logs, check-ins, assessments as the core) → Tracking. 
- Editorial/education-first (articles, tips, psychoeducation with minimal action) → Health_tips. 
- Audio-only libraries (white noise, soundscapes, sleep radio) with little/no instruction → Radio.
  If it’s mainly a tool (timers, sound generator, breathing pacer with no plan/logs), → Tools.
...
- Booking is primary only if the app’s main purpose is reservations/appointments (class/slot/tee time aggregation, matching/scheduling with professionals (therapists, coaches, dietitians, physios), marketplace, or schedule-first UX).
  If booking is just one feature of a gym-chain member app that also provides workout tracking and/or training plans, do not choose Booking.
  If logs/history are emphasized → purpose="Tracking".
  If on-demand plans/tutorials dominate → purpose="Training".
...
2) Wearables/companion fitness: steps/distance/HR/sleep with periodic reports if the app mainly syncs and displays steps/HR/sleep without a workout plan catalog → sport_type="health_metrics", purpose="Tracking", athlete=true.
...
13) Meditation & mindfulness platforms (Calm, Headspace, Balance) → purpose="Training", sport_type="mental_wellbeing", athlete=true.
14) Mood journals & CBT thought records (Daylio, Stoic-style) → purpose="Tracking", sport_type="mental_wellbeing", athlete=true.
15) Sleep soundboards/white noise → purpose="Radio" (or Tools if generator/controls are the core), sport_type="sleep_health", athlete=true.
16) Gym-chain member apps (Planet Fitness, EōS, etc.): sport_type="fitness_gym".
    Tracking logs/history present → purpose="Tracking".
    Workout plans/tutorials but no personal history → Training.
    Booking/classes mentioned but not central → do not use Booking.
17) Teletherapy / telepsychology platforms (e.g., BetterHelp, Talkspace): purpose="Booking", sport_type="mental_wellbeing", athlete=true, support_staff=false, supporter=false, governing_entity=false.
    If the app is primarily a self-guided mindfulness/CBT course library without clinician matching, use purpose="Training" instead.

# Sport type
- Set one primary sport. Use predefined tags if fitting. Create a new tag if needed. Use "various" ONLY if multiple distinct sports/leagues are covered or the sport is unspecified.
...
Common tags: "football" (soccer), "american_football", "basketball", "baseball", "disc_golf", "fitness_gym", "golf",
"tennis","rugby","cricket","hockey","volleyball","bowling","running","cycling", "general_health",
"e_biking","endurance_sports","swimming","rowing","hunting","fishing","hunting_fishing",
"shooting_sports","trapping","motorsport", "mobility_stretching","combat_sports","dance", "physical_therapy_rehab", "yoga","pilates",
"cheerleading","pickleball","equestrian","esports","team_sports","various", "walking", "UNKNOWN".
...
- mobility_stretching — apps whose primary content is flexibility/mobility routines (static/dynamic stretching, ROM work, PNF, foam rolling), not framed as yoga/pilates and without broad cross-training catalogs.
- physical_therapy_rehab (optional) for clinician-led rehab protocols, injury-specific progressions, or post-op plans.

Health & fitness tags: "sleep_health","nutrition_health","hydration_health","menstrual_cycle","pregnancy_perinatal","perimenopause_health","mental_wellbeing","health_metrics","general_health".
Use these only for non-sport health contexts. If the app’s primary content is workouts/plans/tutorials, use fitness_gym (or a specific modality like yoga/pilates, mobility_stretching).
If multiple non-sport facets appear, choose the primary one; if none clearly dominates, use general_health.
Do not use these when training is to improve a specific sport → pick that sport (e.g., tennis).

Avoid inventing new health  fitness tags beyond this list; otherwise use UNKNOWN or general_health.
- fitness_gym: Use for exercise/workout content not tied to a specific sport, including home workouts, strength training, HIIT, bodyweight/calisthenics, machine/free-weight instruction, gym chain member apps, general conditioning, and mixed “get fit/lose weight” programs.
